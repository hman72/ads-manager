...
function App() {
  return (
    <div style={{ padding: '20px' }}>
      <h1>🚀 Ads Manager Test</h1>
      <p>If you see this, React is working!</p>
      <p>The role-based system will be restored shortly...</p>
    </div>
  );
}
...
const ROLE_PERMISSIONS = {
  ...
  [ROLES.CAMPAIGN_MANAGER]: [
    ...
    PERMISSIONS.CREATE_AD_GROUP,
    PERMISSIONS.EDIT_AD_GROUP,
    PERMISSIONS.UPLOAD_CREATIVE,
...
const UserRoleProvider = ({ children }) => {
  const [currentUser, setCurrentUser] = useState({
    ...
    role: ROLES.CAMPAIGN_MANAGER, // Default role - can be changed
    accountAccess: ['all'] // Which client accounts this user can access
  });
...
const PermissionGuard = ({ permission, permissions, fallback = null, children }) => {
  const { hasPermission, hasAnyPermission } = useUserRole();
...
const campaignsData = [
  {
    id: 1,
    campaign: "Agency | Zenith US | Walmart | DI | WTYW Discovery Q3'24-Q4'24 | Ad Buy - US",
    goal: "Conversion",
...
function AppContent() {
  // Role management
  const { currentUser, hasPermission, hasAnyPermission, switchRole, setCurrentUser } = useUserRole();
  ...
  const handleMultipleAdGroupsInfoDrawerOpen = () => {
  ...
  const CampaignDetails = ({ 
    ...
    const [lifetimeBudget, setLifetimeBudget] = useState("125000");
    ...
    <Container maxWidth={false} sx={{ mt: 0, p: "20px" }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <Button 
              variant="outlined" 
              size="small"
              onClick={() => setCampaignEditDrawerOpen(true)}
            >
              Edit
            </Button>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, py: '12px' }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
          <Button variant="outlined" size="small" onClick={handleMultipleAdGroupsInfoDrawerOpen}>
  ...
  const AdGroupDetails = ({ adGroup, selectedTimeSlots }) => {
          ...
          {(adGroup.status === 'Active' || adGroup.status === 'Ended') && (
            <Box sx={{ 
              ...
              <Box>
                ...
                <Typography variant="body1" sx={{ fontWeight: 'bold' }}>{adGroup.spend}</Typography>
              </Box>
              <Box>
                <Typography variant="caption" color="text.secondary">Impressions</Typography>
              ...
              <Box sx={{ mt: 2 }}>
                ...
                <TableContainer component={Paper} sx={{ maxHeight: 200 }}>
                  <Table size="small" stickyHeader>
                    <TableHead>
                      <TableRow>
                        <TableCell sx={{ fontWeight: 'bold', backgroundColor: 'grey.50' }}>Day</TableCell>
                        <TableCell sx={{ fontWeight: 'bold', backgroundColor: 'grey.50' }}>Time Range</TableCell>
                      </TableRow>
                    </TableHead>
                    <TableBody>
                      {(() => {
                        ...
                        Array.from(selectedTimeSlots).forEach(slot => {
                          const [day, hour] = slot.split('-');
                          if (!dayRanges[day]) {
                            dayRanges[day] = [];
                          }
                          dayRanges[day].push(parseInt(hour));
                        });
                        
                        // Convert to ranges for each day
                        return Object.entries(dayRanges).map(([day, hours]) => {
                          hours.sort((a, b) => a - b);
                          ...
                          return (
                            <TableRow key={day}>
                              ...
                              <TableCell>{ranges.join(', ')}</TableCell>
                            </TableRow>
                          );
                        });
                      })()}
    ...
    <>
          ...
          <Box sx={{ mr: 2, display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            <FormControl size="small" sx={{ minWidth: 140 }}>
              ...
              >
                ...
                <MenuItem value={ROLES.CAMPAIGN_MANAGER}>Campaign Manager</MenuItem>
      ...
      {currentView === 'list' ? (
        <Container maxWidth={false} sx={{ mt: 4, p: "20px" }}>
          ...
          {/* Role-Based Permissions Demo Panel */}
          <Box sx={{ 
            p: 2, 
            mb: 3, 
            backgroundColor: 'rgba(32, 0, 76, 0.05)', 
            border: '1px solid rgba(32, 0, 76, 0.1)',
            borderRadius: 2 
          }}>
            <Typography variant="h6" sx={{ mb: 2, color: '#20004c', fontWeight: 'bold' }}>
              🔐 Role-Based Access Control Demo
            </Typography>
            <Grid container spacing={3}>
              <Grid item xs={12} md={6}>
                <Typography variant="subtitle2" sx={{ mb: 1, fontWeight: 'bold' }}>
                  Current Permissions ({currentUser.role.replace('_', ' ').toUpperCase()}):
                </Typography>
              ...
              <Grid item xs={12} md={6}>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    ...
                    <Typography variant="body2">Create Campaigns</Typography>
                  </Box>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    {hasPermission(PERMISSIONS.VIEW_BUDGET) ? '✅' : '❌'}
                    <Typography variant="body2">View Budget Data</Typography>
                  </Box>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    {hasPermission(PERMISSIONS.EDIT_BUDGET) ? '✅' : '❌'}
                    <Typography variant="body2">Edit Budgets</Typography>
                  </Box>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    {hasPermission(PERMISSIONS.APPROVE_CREATIVE) ? '✅' : '❌'}
                    <Typography variant="body2">Approve Creatives</Typography>
                  </Box>
                </Box>
              </Grid>
            </Grid>
              ...
              <Tab 
                label={
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    Creatives
                    <Chip 
                      label={selectedCampaigns[2]?.length > 0 ? `${selectedCampaigns[2].length} selected` : getTabCount()} 
                      size="small" 
                      sx={{ 
                        height: '20px',
                        fontSize: '12px',
      ...
      >
        <Box sx={{ p: 3 }}>
          ...
          <Typography variant="h5" sx={{ mb: 3 }}>
            {drawerContext === 'locations' ? 'Select locations for your ad campaign' : "What's your advertising objective?"}
          </Typography>
          
          {drawerContext === 'locations' && (
            <Box>
              <Typography variant="body1" sx={{ mb: 2 }}>
                Choose where you want your ads to be shown
              </Typography>
              <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
                Location targeting options and settings will be displayed here.
              </Typography>
            </Box>
          )}
            ...
            <Button 
              ...
              onClick={() => {
                ...
                const newCampaign = {
                  ...
                  goal: selectedCampaignType === 'awareness' ? 'Awareness' : 'Conversion',
                  ...
                  goalActions: "0",
                  goalCpa: "$0.00",
                  lastModified: new Date() // Set current timestamp for newly created campaigns
                };
                ...
                // Create corresponding ad group with same name
                const newAdGroup = {
                  id: adGroups.length + 1, // Simple ID generation
                  campaign: campaignName, // Same name as campaign
                  parentCampaign: campaignName, // Belongs to the new campaign
                  status: "Draft",
                  spend: "$0.00",
                  impressions: "0",
                  cpm: "$0.00",
                  goalActions: "0",
                  goalCpa: "$0.00",
                  lastModified: new Date() // Set current timestamp for newly created ad groups
                };
                
                // Add to ad groups list
                setAdGroups(prev => [...prev, newAdGroup]);
                
                // Set as selected campaign for details view
      ...
      >
        ...
        <Box sx={{
          ...
          <Box sx={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: 2, maxWidth: 'none', mx: 'auto' }}>
            ...
            <PermissionGuard 
              permission={PERMISSIONS.CREATE_AD_GROUP}
              fallback={
                <Button 
                  variant="contained" 
                  disabled
                  size="medium"
                  title="You don't have permission to create ad groups"
                >
                  Create Ad Group
                </Button>
              }
          ...
          <Box sx={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: 2, maxWidth: 'none', mx: 'auto' }}>
            ...
            <Button 
              ...
              onClick={() => {
                ...
                const adGroupName = newAdGroupName.trim() || 'New Ad Group';
                const campaignName = campaigns[0]?.campaign || 'Default Campaign';
                ...
                // Create a second ad group that matches the campaign name
                const campaignAdGroup = {
                  id: Date.now() + 1, // Simple ID generation for demo
                ...
                // Add both ad groups to the adGroups array
                setAdGroups(prev => [...prev, newAdGroup, campaignAdGroup]);
                ...
                setSelectedCampaignForDetails(null); // Clear campaign selection
                setSelectedAdGroupForDetails(newAdGroup);
                setCurrentView('details');
              }}
            >
                ...
                {[0, 1, 2, 3, 4, 5, 6].map(dayKey => (
                  <Box key={dayKey} sx={{ display: 'flex' }}>
                    {Array.from({ length: 24 }, (_, hour) => {
                      const cellKey = `${dayKey}-${hour}`;
                      const isSelected = selectedTimeSlots.has(cellKey);
